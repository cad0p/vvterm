...
//  iotest
//
//  Session 1.9 — port of Teleport's `services.NewHeadlessAuthenticationID`
...
//
//  CI-able: the FixtureTests target compares this against a Go-generated
//  fixture (see spikes/sep-webauthn/fixtures/generate/main.go) so a
//  transcription error is caught without a device.
//
...
enum HeadlessID {
    ...
    // MARK: - Self-check (CI-able fixture comparison)

    /// The fixed test vector — MUST match the Go fixture generator
    /// (spikes/sep-webauthn/fixtures/generate/headless/main.go) exactly.
    /// This is a synthetic ed25519 SSH public key (32 bytes of 0xff in the
    /// raw pubkey field) with a trailing newline to match
    /// ssh.MarshalAuthorizedKey output.
    static let fixtureSSHPubKey =
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIP////////////////////////////////////8 test"

    /// The expected headless ID for `fixtureSSHPubKey`, generated by the
    /// Go fixture generator. Loaded from the bundled fixture file at runtime.
    static func fixtureExpectedID() -> String? {
        // The fixture is bundled as a resource in the iotest app.
        // (See the Resources group in the Xcode project.)
        guard let url = Bundle.main.url(forResource: "headless_id", withExtension: "txt"),
    ...
    /// Run a self-check: compute the headless ID for the fixture SSH pub key
    /// and compare against the Go-generated fixture. Returns true if they match.
    /// Emits the `[IOTEST] headless_id_fixture_match=true|false` marker.
    static func selfCheck() -> Bool {
        let computed = compute(sshAuthorizedKey: fixtureSSHPubKey)
        let expected = fixtureExpectedID() ?? "<fixture not found>"
        let match = (computed == expected)
        HeadlessLog.logger.notice("[IOTEST] headless_id_fixture_match=\(match ? "true" : "false")")
        HeadlessLog.logger.notice("[IOTEST] headless_id_fixture_computed=\(computed, privacy: .public)")
        HeadlessLog.logger.notice("[IOTEST] headless_id_fixture_expected=\(expected, privacy: .public)")
        return match
    }
}
