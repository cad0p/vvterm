...
//  iotest
//
//  Session 1.11 — the Browser MFA assertion ceremony.
//
...
enum BrowserMFACeremonyLog {
    static let logger = Logger(subsystem: "it.pcad.vvterm.iotest", category: "browser-mfa-ceremony")

    static func step(_ step: String, _ message: String) {
        logger.notice("[IOTEST] bmfa_ceremony_\(step, privacy: .public) \(message, privacy: .public)")
    }
}
...
/// Safari must be on the main thread) and publishes to SwiftUI.
@MainActor
final class BrowserMFACeremony: NSObject, ObservableObject {
    ...
    /// A log callback so the runner can surface ceremony progress to the UI.
    ...
    func run(conn: TeleportGRPCConnection, host: String) async throws -> Proto_BrowserMFAResponse {
        ...
        log("Browser MFA: listener on \(clientCallbackURL)")
        BrowserMFACeremonyLog.step("listener", clientCallbackURL)
        ...
        BrowserMFACeremonyLog.step("create_auth_challenge", "ContextUser MANAGE_DEVICES + BrowserMFATSHRedirectURL")
        log("Browser MFA: CreateAuthenticateChallenge (with BrowserMFATSHRedirectURL)…")
        ...
        BrowserMFACeremonyLog.step("got_challenge", "request_id=\(requestID.prefix(16))…")
        log("Browser MFA: got challenge (request_id=\(requestID.prefix(16))…)")

        // ── 4. Open Safari to /web/mfa/browser/<id> ───────────────────────
        let browserMFAURL = "https://\(host)/web/mfa/browser/\(requestID)"
        BrowserMFACeremonyLog.step("open_safari", browserMFAURL)
        log("Browser MFA: opening Safari to /web/mfa/browser/<id> (assert the iCloud passkey)…")
        ...
        BrowserMFACeremonyLog.step("got_callback", "webauthn id=\(webauthnResp.id.prefix(16))…")
        log("Browser MFA: listener received callback (webauthn id=\(webauthnResp.id.prefix(16))…)")
        ...
        BrowserMFACeremonyLog.step("done", "ExistingMFAResponse.Browser built")
        log("Browser MFA: assertion complete, ExistingMFAResponse.Browser built")
        return response
        #else
    ...
    private func openSafari(url: URL) async {
        return await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            let session = ASWebAuthenticationSession(url: url, callbackURLScheme: "vvterm") { _, error in
                ...
                if let error {
                    BrowserMFACeremonyLog.step("safari_callback", "error: \(error.localizedDescription)")
                } else {
                    BrowserMFACeremonyLog.step("safari_callback", "session ended (dismissed or redirected to vvterm://)")
                }
            }
            ...
            BrowserMFACeremonyLog.step("safari_started", "\(started)")
