...
- **Flaky tests are a performance bug, not just a correctness bug.** UI shards run with a 300s per-test execution allowance (`-test-timeouts-enabled YES`, placed **before** `-parallel-testing-enabled` — the latter otherwise eats the next option token and silently disables the timeouts): a hanging test is killed and recorded as a failure **by name** (`Exceeded execution time allowance`) instead of wedging the shard. Treat every such failure as a hang to fix or quarantine, not as a slow test. 300s is 1.2× the longest legitimate test (`ServerNavigationUITests.testActiveTerminalPushPopPreservesListPositionAndSession`, 185.8–245.1 s across five 2026-09-23 runs; the next longest is ~98 s). When a shard's runtime is well above its peers, suspect a hang. File a GitHub Issue, and either fix the flake, mark `XCTSkip` with a reference to the issue, or quarantine the test into a separate non-blocking job until fixed. Do not let flakes persist in the hot path.
