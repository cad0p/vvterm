...
- **The UI shards are not required checks.** `gh-ruleset-main` requires only `build` and `unit-tests` (owner decision 2026-09-25, recorded on #255). The four `ui-tests-shard-*` jobs still run on every PR and still go red when a test fails — they **report, they do not gate**. This is measured, not theoretical: with the retry loop deleted, a run whose only delta was comment and documentation text hit two host-state wedges in two different shards (`Failed to terminate` inside XCTest's own `XCUIApplication.launch()`, and the `#201` AX stale-frame wedge). A red shard is therefore a **triage signal, not a merge blocker** — fix or quarantine the class, and never re-add retry machinery. The AX/lifecycle family is tracked on #201.
