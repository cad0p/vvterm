...
- `VVTerm PR OTA` (single `ota-archive` job, split out of PR CI for the ios-adhoc-pr trigger latency) shares the same 5-slot pool. It overlaps the test phase for ~5 min (shards start ~7m, ota-archive ends ~12m), so one shard may queue briefly. Accepted trade for the ~15-min push→installable-comment path (pcad.it-infra #154); do not "fix" it by serializing `ota-archive` behind tests.
