...
- GitHub Actions caps this repo at **5 concurrent macOS jobs**. The workflow is sized to fit that cap with zero queueing: `build` (1) runs first, then `unit-tests` (1) + 4 `ui-tests` shards (4) = 5 in parallel. Do NOT add a 5th UI shard — it would queue behind a finished job and add its full runtime to the wall-clock. The 2026-09-23 LPT split (78 executing methods at measurement time, 3 loopback-fixture tests per bin) ran 517.8–527.8 s of test time per shard (max/min 1.02) + ~322 s setup ≈ 14m/shard, so the wall clock is build + the slowest shard ≈ 17m median; #227 has since split the push/pop test into two methods, both kept in shard-0, pending the ≥3-clean-run re-measurement.
- **The UI shards are not required checks.** `gh-ruleset-main` requires only `build` and `unit-tests` (owner decision 2026-09-25, recorded on #255). The four `ui-tests-shard-*` jobs still run on every PR and still go red when a test fails, so a red shard stays visible in the PR's check list — but it does not veto the merge. This is measured, not theoretical: with the retry loop deleted, a run whose only delta was comment and documentation text hit two host-state wedges in two different shards (`Failed to terminate` inside XCTest's own `XCUIApplication.launch()`, and the AX stale-frame wedge). So treat a red shard as a **triage signal, not a merge blocker**: classify it, record it with its run/job URL on **#257** (the host-state flake tracker — #201 is closed history, and #119, #220, #232 and #252 are the open per-test instances), and fix or quarantine the class. Never re-add retry machinery.
- `VVTerm PR OTA` (`vvterm-pr-ota.yml`, single `ota-archive` job, no `needs:`, starts at push time) shares the same 5-slot pool and completes ~9-13m after push; its completion triggers `ios-adhoc-pr` (workflow_run on main — the trusted sign + R2 upload + sticky-comment pipeline, never touches PR code). One test job may transiently queue behind the archive; accepted.
- **OTA publish FIFO gate.** Because `ios-adhoc-pr`'s `sign-and-publish` job is created ~10-13m after push, the FIFO runner queue used to place it behind all 5 test jobs (measured 4-15m queue). Fix: `unit-tests` has `needs: [build, wait-for-ota-publish]`; the `wait-for-ota-publish` gate job (ubuntu, no macOS slot) releases it once `sign-and-publish` has started, so the installable build lands ~1m after the archive. The gate degrades gracefully (missing/failed/timed-out OTA or publish → unit-tests proceeds immediately). Never gate the UI shards on the OTA pipeline — they are the wall-clock critical path. Residual: a concurrent external account-level macOS job (`ios-testflight` main push, teleport-e2e dispatch) can still delay the publish; a 3-shard consolidation would fit 3 shards + OTA + one external job under the cap but is not planned while 4 shards stay inside the 45m job cap (see #110 for the costed decision).
...
- **Flaky tests are a performance bug, not just a correctness bug.** UI shards run with a 300s per-test execution allowance (`-test-timeouts-enabled YES` with explicit values, placed **before** `-parallel-testing-enabled NO` — a bare `-parallel-testing-enabled` otherwise eats the next option token and silently disables the timeouts): a hanging test is killed and recorded as a failure **by name** (the serial shard prints `Test Case '…' exceeded execution time allowance of N minutes.`, and older/parallel runs print `failed … (300.000 seconds)`) instead of wedging the shard. **Shards do not retry** — a failed shard fails the run, and a per-test execution-allowance kill is a verdict, not an infra signature. A flake must therefore be fixed (with a deterministic test or gate) or quarantined via `XCTSkip`, never retried away. The one failure mode that is not a test defect is a **pre-test runner infra wedge** — a shard that fails with zero `Test Case` lines in its `test.log` (`Runner failed before any test executed`): that is runner infrastructure, so re-run the job (`gh run rerun --failed`) rather than fixing or quarantining a healthy test. **Parallel testing is explicitly OFF** — the 4-bin LPT split is computed from serial per-test sums, and `-parallel-testing-enabled YES` runs 2 simulator clones per shard (8 concurrent across the shards), which correlates with the wedge wave of 2026-09-24; serial mode also keeps XCTest's per-test diagnostics (`XCTAssert`, `scene=`, `sentCount=`) and the classic `Test Case '-[…]'` block boundaries on stdout. Treat every allowance kill as a hang to fix or quarantine, not as a slow test. 300s is 1.2× the longest legitimate test (`ServerNavigationUITests.testActiveTerminalPushPopPreservesListPositionAndSession`, 185.8–245.1 s across five 2026-09-23 runs at measurement time; the next longest is ~98 s — #227 has since split this test into a list-position half and a session half), but a degraded runner can still push a long test past 300s — that kill is the allowance working, and the split allows a tighter allowance to be re-derived once the ≥3-clean-run re-measurement lands. When a shard's runtime is well above its peers, suspect a hang. File a GitHub Issue, and either fix the flake or mark it `XCTSkip` with a reference to the issue. There is no separate non-blocking job to move a test into: the shards themselves no longer gate, and a 5th UI job would break the 5-job cap. Do not let flakes persist in the hot path.
- **Shorten tests that take too long.** A single UI test over ~60s is a candidate for optimization: reduce sleep/animation waits, avoid full app relaunches where a view-harness suffices, split into smaller focused tests, or move the logic to a unit test. LPT already pairs the slowest tests with the smallest ones; if one test dominates a bin (like `testActiveTerminalPushPopPreservesListPositionAndSession` at 185.8–245.1 s, split by #227 into a list-position half and a session half), split the test rather than inventing an outlier shard.
...
- **Teleport E2E (`teleport-e2e.yml`) stays OUT of PR CI.** The real-Teleport integration tests (issue #83) live in `VVTermTests` as env-gated tests (`VVTERM_TELEPORT_*` set → run, absent → skip), so they cost ~0s in the normal unit-tests job. The actual real-server run happens in a separate dispatch/schedule workflow on the `xcode-27` runner (`scripts/ci/teleport-server.sh` boots an all-in-one Teleport cluster on the same runner — GHA jobs can't reach each other, and the simulator shares the host loopback). Do NOT add a per-PR teleport job: it would break the 5-concurrent-job zero-queueing invariant.
