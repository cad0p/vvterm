...
- GitHub Actions caps this repo at **5 concurrent macOS jobs**. The workflow is sized to fit that cap with zero queueing: `build` (1) runs first, then `unit-tests` (1) + 4 `ui-tests` shards (4) = 5 in parallel. Do NOT add a 5th UI shard — it would queue behind a finished job and add its full runtime to the wall-clock. The 2026-09-23 LPT split (78 executing methods, 3 loopback-fixture tests per bin) runs 517.8–527.8 s of test time per shard (max/min 1.02) + ~322 s setup ≈ 14m/shard, so the wall clock is build + the slowest shard ≈ 17m median.
