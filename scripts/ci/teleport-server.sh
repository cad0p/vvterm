...
# ---------------------------------------------------------------------------
# Fetch the certificate the proxy's web listener serves on the local dial
# address. `-servername` keeps the TLS routing SNI on the default (web)
# route; LibreSSL/openSSL both parse the first certificate from s_client.
fetch_web_cert() {
  openssl s_client -connect "${TELEPORT_HOST}:${TELEPORT_WEB_PORT}" \
    -servername "${TELEPORT_HOST}" </dev/null 2>/dev/null \
    | openssl x509 -outform PEM
}

write_env_file() {
  # The proxy's web-listener certificate. When no `https_keypairs` are
  # configured the proxy serves a self-signed certificate whose SANs cover
  # the local host (e.g. 127.0.0.1); the app's webapi leg verifies with
  # system trust, so the CI simulator adds this certificate to its root
  # store before the ceremony test. Best-effort: an unreachable endpoint
  # leaves it empty and the ceremony test fails on the certificate as before.
  local web_cert_pem=""
  if web_cert_pem="$(fetch_web_cert 2>/dev/null)" && [ -n "${web_cert_pem}" ]; then
    printf '%s\n' "${web_cert_pem}" > "${FIXTURES_DIR}/web-cert.pem"
  else
    log "warning: could not fetch the web-listener certificate from https://${TELEPORT_HOST}:${TELEPORT_WEB_PORT} — VVTERM_TELEPORT_WEB_CERT will be empty"
  fi
  cat > "${ENV_FILE}" <<EOF
...
# The proxy's web-listener certificate (PEM), exported by write_env_file and
# written to $FIXTURES_DIR/web-cert.pem. The app's webapi leg verifies with
# system trust; consumers that need the ceremony against this cluster add
# this certificate to their trust store first (the CI simulator does).
VVTERM_TELEPORT_WEB_CERT="${web_cert_pem}"
EOF
  log "wrote ${ENV_FILE}"
}
